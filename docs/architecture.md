...
│                                                             │
│  MinMaxPreselect: 等宽 x 区间分桶 → 每桶 argmin/argmax(y)    │
...
整个扩展的实现集中在单个文件 `src/lttb_extension.cpp`（约 1380 行）中，按功能分为以下几个部分：
...
static std::vector<idx_t> MinMaxPreselect(...);             // MinMax 预选（minmax_lttb）
```
...
LTTB 扩展的全部逻辑约 1380 行，拆分为多个文件会引入不必要的头文件依赖和编译复杂度。单文件结构使得：
...
│   └── lttb_extension.cpp         # 全部实现（约 1380 行）
├── test/
│   └── sql/
