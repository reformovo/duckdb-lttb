...
│              DuckDB 函数绑定层 (Bind)                        │
│                                                             │
│  LTTBBindFunctionImpl                                       │
│    ├─ 校验 x/y 类型是否在支持集合内                           │
│    ├─ SQLNULL → DOUBLE 强制转换（向后兼容）                   │
│    ├─ 设置 function.arguments 为具体类型                     │
│    ├─ 设置 function.SetReturnType 为 LIST(STRUCT(x,y)[])    │
│    └─ 返回 LTTBFunctionData (携带 sorted 标志)              │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
│                                                             │
│  Update:   ReadAsDouble 将类型化输入转为 double 存入 state   │
│  Combine:  指针转移或 reserve+insert 合并部分状态             │
│  Finalize: Downsample 执行 LTTB 算法                         │
│            WriteFromDouble 将 double 结果转回类型化输出       │
...
核心设计原则：**算法在 `double` 上运行，类型转换只发生在 I/O 边界**。这使得 LTTB 算法本身与输入类型完全解耦，类型保持成为一个独立的 I/O 关注点。

---

## 3. 代码结构

整个扩展的实现集中在单个文件 `src/lttb_extension.cpp`（682 行）中，按功能分为以下几个部分：

### 3.1 数据结构定义（第 22-50 行）

```cpp
struct LTTBPoint { double x, y; };           // 算法内部的数据点
struct LTTBState { ... };                     // 聚合状态
struct LTTBFunctionData : public FunctionData { ... };  // Bind 数据
```

### 3.2 类型转换辅助函数（第 56-262 行）

```cpp
static double ReadAsDouble(...);              // 类型化输入 → double
static void WriteFromDouble(...);             // double → 类型化输出
static bool IsLTTBSupportedType(...);         // 类型校验
```

### 3.3 LTTB 算法核心（第 264-376 行）

```cpp
static std::vector<LTTBPoint> Downsample(...); // 降采样核心逻辑
```

### 3.4 聚合生命周期函数（第 378-565 行）

```cpp
static void LTTBInitialize(...);   // 状态初始化
static void LTTBUpdate(...);       // 逐行更新状态
static void LTTBCombine(...);      // 合并部分状态（并行执行时）
static void LTTBFinalize(...);     // 最终化：执行降采样并写入结果
static void LTTBIndicesFinalize(...); // 最终化：写入索引结果
static void LTTBDestroy(...);      // 释放状态内存
```

### 3.5 函数注册与绑定（第 567-658 行）

```cpp
static LogicalType LTTBReturnType(...);                    // 构造返回类型
static AggregateFunction GetLTTBConcreteFunction(...);     // 点输出具体函数
static AggregateFunction GetLTTBIndicesConcreteFunction(...); // 索引输出具体函数
static unique_ptr<FunctionData> LTTBBindFunctionImpl(...); // 统一绑定逻辑
static AggregateFunction GetLTTBFunction(...);             // 占位函数（ANY,ANY,BIGINT）
// ... 以及 sorted/indices 变体的占位函数
static void LoadInternal(...);                             // 注册所有函数
```

### 3.6 扩展入口（第 660-682 行）
...
LTTB 扩展的全部逻辑约 680 行，拆分为多个文件会引入不必要的头文件依赖和编译复杂度。单文件结构使得：
...
### 4.1 类型保持 I/O：ReadAsDouble / WriteFromDouble
...
**ReadAsDouble**（第 60-138 行）通过 `switch (type.id())` 分发，对每种类型使用对应的 DuckDB API 读取物理值并转为 `double`：
...
| `DECIMAL` | int16/int32/int64/hugeint_t | 读原始整数 ÷ 10^scale |
| 整数类型 | 对应宽度整数 | `static_cast<double>` |
| `HUGEINT` / `UHUGEINT` | 128 位整数 | `Hugeint::TryCast` / `Uhugeint::TryCast` |

**WriteFromDouble**（第 142-235 行）是逆操作，将 `double` 转回目标类型写入输出向量。DECIMAL 写回时使用 `std::llround(value * 10^scale)` 做四舍五入。

**为什么用 switch 而不是模板**：DuckDB 的 `first`/`last` 等聚合函数使用模板为每种类型生成独立的函数实例，因为它们的状态需要存储类型化值。但 LTTB 的状态始终是 `double`，不需要为每种类型生成独立的 Update/Combine/Finalize 函数。单个 switch 分发更简洁，避免了模板膨胀，且类型在循环外读取一次，switch 分支预测器能很好地处理循环内的重复分发。

**单调性保证**：所有支持的类型到 `double` 的转换都是单调的——即 `a < b` 在原始类型中成立，当且仅当 `ReadAsDouble(a) < ReadAsDouble(b)` 在 `double` 中成立。这保证了在 `double` 上排序的结果与在原始类型上排序的结果一致。代码中在排序处添加了注释说明这一不变量。

### 4.2 LTTB 算法：Downsample

`Downsample`（第 270-376 行）实现了 LTTB 降采样的核心逻辑，与 ClickHouse PR #57003 修复后的行为一致：
...
**LTTBCombine**（第 449-488 行）——并行执行时合并部分状态：
...
**LTTBBindFunctionImpl**（第 592-617 行）是统一的绑定逻辑：
...
`FunctionData` 是 DuckDB 的绑定数据基类，用于在绑定阶段携带信息到执行阶段。`Copy()` 和 `Equals()` 是 DuckDB 要求的虚函数，用于查询计划缓存和等价性检查。`sorted` 标志在 Finalize 中被读取，决定是否跳过排序。
...
**DuckDB 的改进**：稳定排序、空桶保护、NULL 处理、排序快速路径、索引输出。

**DuckDB 的待改进项**：Arena 集成、minmax_lttb 近似路径、用户可配置内存上限。

---

## 8. 测试策略

测试使用 DuckDB 的 SQLLogicTest 框架，集中在 `test/sql/lttb.test`（99 个断言）。
...
│   └── lttb_extension.cpp         # 全部实现（682 行）
├── test/
│   └── sql/
│       └── lttb.test              # SQLLogicTest 测试（99 断言）
├── docs/
│   ├── reference/
...
1. **minmax_lttb 近似路径**：参考 `plotly-resampler` 的 `MinMaxLTTB`，先对粗粒度桶保留 first/last/min/max，再对候选集运行 LTTB。适用于 >1M 样本的交互式可视化场景。
2. **基准测试**：添加 100K 和 1M 点的单序列基准，跟踪排序、采样、输出的成本。
3. **用户可配置内存上限**：通过 PRAGMA 设置或 FunctionData 传递每组最大点数，用于生产环境的大查询保护。
4. **Arena 集成**：将状态分配迁移到 DuckDB 的 ArenaAllocator，减少堆分配碎片。
